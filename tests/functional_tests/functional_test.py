...
class FunctionalTest(unittest.TestCase):
    ...
    def run_configuration(self, path_to_test, output_folder, output_saved_folder, model_xml, data_set_xml, optimization_parameters_xml, command='estimate', precision=DEFAULT_PRECISION):
        ...
        #           'bash -c \'conda activate deformetrica && python %s %s %s --dataset=%s --output=%s -v DEBUG > %s\'' % \
        ...
        #     # without dataset
        #     cmd = 'if [ -f ~/.profile ]; then . ~/.profile; fi && ' \
        #           'bash -c \'conda activate deformetrica && python %s %s %s --output=%s -v DEBUG > %s\'' % \
        ...
        if command in ['estimate', 'initialize']:
            cmd = 'if [ -f ~/.profile ]; then . ~/.profile; fi && ' \
                  'bash -c \'conda activate deformetrica && python %s %s %s %s --parameters=%s --output=%s -v DEBUG > %s\'' % \
            ...
            # without dataset
            cmd = 'if [ -f ~/.profile ]; then . ~/.profile; fi && ' \
                  'bash -c \'conda activate deformetrica && python %s compute %s --parameters=%s --output=%s -v DEBUG > %s\'' % \
                  (path_to_deformetrica, path_to_model_xml, path_to_optimization_parameters_xml, path_to_output, path_to_log)
        else:
